...
def start_game(blenderoptions='', gameoptions=''):
	...
	config = read_config()
	width = config['window_width'] if 'window_width' in config else 1280
	height = config['window_height'] if 'window_height' in config else 800
	if sys.platform in ('linux', 'linux2'):
		with open(bge.logic.expandPath('//../blenderplayer_path.txt'), 'r') as f:
...
def mouse_over_item(cont):
	...
	if mouseover.status == KX_SENSOR_JUST_ACTIVATED:
		...
		# play sound
		sound = aud.Factory(f'{sound_path}/share/interface-rollover.mp3')
		audio.volume = 0.1
		audio.play(sound)
...
def root_select():
	...
	if active_element == 'root':	
		...
		if name == "enter_local_grid":
			...
			bge.logic.endGame()
		# play sound
		sound = aud.Factory(f'{sound_path}/share/menu-button.mp3')
		audio.volume = 0.3
		audio.play(sound)
...
def keyboard(cont):
	...
	if active_element == 'root':
		...
		if (keyboard.getKeyStatus(UPARROWKEY) == KX_INPUT_JUST_ACTIVATED
		    and root_item_selected < len(root_items) - 1):
			root_item_selected += 1
		if (keyboard.getKeyStatus(DOWNARROWKEY) == KX_INPUT_JUST_ACTIVATED
		    and root_item_selected > 0):
			root_item_selected -= 1
		...
		if keyboard.getKeyStatus(ESCKEY) == KX_INPUT_JUST_ACTIVATED:
			...
			active_element = 'net_user'
		elif [TABKEY, KX_INPUT_JUST_ACTIVATED] not in keyboard.events:
...
def text_enter(keyboard, cursor, entry):
	...
	if keyboard.getKeyStatus(LEFTARROWKEY) == KX_INPUT_JUST_ACTIVATED:
		...
		place = entry['cursor']
		text = entry['Text']
		entry['Text'] = f'{text[:place]}.{text[place:]}'
		entry['cursor'] += 1

	else: # threat all characters
		shift = cursor['capslock']
		for event in keyboard.events:
			if event[0] in [RIGHTSHIFTKEY, LEFTSHIFTKEY]: shift = not shift
		for event in keyboard.events:
			if event[1] == KX_INPUT_JUST_ACTIVATED:
				if char := EventToCharacter(event[0], shift):
					play_sound = True
					place = entry['cursor']
					text = entry['Text']
